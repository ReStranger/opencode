...
You can configure specialized agents for specific tasks through the `agents` option.
...
```jsonc title="opencode.jsonc"
{
  ...
  "agents": {
    ...
    "code-reviewer": {
      ...
      "system": "You are a code reviewer. Focus on security, performance, and maintainability.",
      "mode": "subagent"
    }
  }
}
...
`model` selects the primary model for the agent. `fallback` is an optional ordered list of alternate models in the same `provider/model` format. OpenCode tries the primary model first, then falls back through the list in order when the selected model is unavailable or the provider turn fails before any assistant output is emitted.
