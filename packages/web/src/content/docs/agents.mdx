...
`fallback` is optional and uses the same `provider/model-id` format. OpenCode tries each fallback in order after the primary model when the selected model cannot be used.
